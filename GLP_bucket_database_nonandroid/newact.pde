boolean actinWasFocused, actinIsFocused, hoursinWasFocused, hoursinIsFocused, minsinWasFocused, minsinIsFocused;

void newact(int value) {

 
  //text entry boxes defined in function drawtextbox() and called in setup (have to be called outside of draw). Visibility controlled by mouseclicked in page selection routine.
  background(255);
  noStroke();
  String textinput = "";
  drawheader();

// trying to add labels to textboxes before click - textbox is always on top, so can't show inside!
//  fill(0);
//  textFont(font24,14);
//  text("hh",112  ,209  );
//  text("mm", 164  ,209  );
...
//      println("outside button");
  ...
  //Check whether the text fields have focus to display instruction text
  actinIsFocused = actin.get(Textfield.class,"actin").isFocus();
  hoursinIsFocused = hoursin.get(Textfield.class,"hoursin").isFocus();
  minsinIsFocused = minsin.get(Textfield.class,"minsin").isFocus();
  
  //get the current values from the field (this would also be useful when doing autocomplete on some future version
  String activityTextTemp = actin.get(Textfield.class,"actin").getText();
  String hourTextTemp = hoursin.get(Textfield.class,"hoursin").getText();
  String minTextTemp = minsin.get(Textfield.class,"minsin").getText();
  
  if ((actinIsFocused && !actinWasFocused) && activityTextTemp.equals("New Activity")){
    actin.get(Textfield.class,"actin").setText("");
    actin.get(Textfield.class,"actin").setColor(0);
  }
  else if ((!actinIsFocused) && activityTextTemp.equals("")){
    actin.get(Textfield.class,"actin").setText("New Activity");
    actin.get(Textfield.class,"actin").setColor(128);
  }

  if ((hoursinIsFocused && !hoursinWasFocused) && hourTextTemp.equals("hh")){
    hoursin.get(Textfield.class,"hoursin").setText("");
    hoursin.get(Textfield.class,"hoursin").setColor(0);
  }
  else if ((!hoursinIsFocused) && hourTextTemp.equals("")){
    hoursin.get(Textfield.class,"hoursin").setText("hh");
    hoursin.get(Textfield.class,"hoursin").setColor(128);
  }

  if ((minsinIsFocused && !minsinWasFocused) && minTextTemp.equals("mm")){
    minsin.get(Textfield.class,"minsin").setText("");
    minsin.get(Textfield.class,"minsin").setColor(0);
  }
  else if ((!minsinIsFocused) && minTextTemp.equals("")){
    minsin.get(Textfield.class,"minsin").setText("mm");
    minsin.get(Textfield.class,"minsin").setColor(128);
  }
  
  
  actinWasFocused = actinIsFocused;
  hoursinWasFocused = hoursinIsFocused;
  minsinWasFocused = minsinIsFocused;

...
//     
     ...
     for (int r = 0; r < myActivitiesToday.length ; r++) {    
        row = r +1;
        cellYpos = (126)+((row)*shiftcellYpos);   
        text(myActivitiesToday[r].timeIn_h+ ":" + (myActivitiesToday[r].timeIn_m < 10? "0":"") + myActivitiesToday[r].timeIn_m,timecellXpos,cellYpos);  //is myActivitiesToday[r].timeIn_m < 10? If so, put in a "0" otherwise, put in nothing.
     ...
     for (int s = 0; s < myActivitiesToday.length ; s++) {    
        ...
        if (myActivitiesToday[s].connectApplic == true)
        {
          switch (myActivitiesToday[s].connectScore) {
            ...
            fill(255); 
            rect(connectcellXpos,cellYpos-4,67 , 20);   //changed to make spreadsheet look ok - need to fix rect positions to overlap correctly for all values!
          }
